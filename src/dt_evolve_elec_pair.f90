...
subroutine dt_evolve_elec_pair
  ...
  do iexp = 1,NTaylor
    zfact = zfact*(-zI*dt*0.5d0)/iexp
    call hpsi_new_pair2(zCt_t,zhCt_t)
    zCp = zCp + zfact*zhCt_t
    zCt_t = zhCt_t
  end do
...
!=======================================================================
subroutine hpsi_pair2(zCt_t,zhCt_t)
  use global_variables
  implicit none
  integer :: i
  ...
  zhCt_t=-gamma*sqrt(2d0*mass*omega0)*0.5d0*(Xp_HO + X_HO)*zCt_t
...
end subroutine hpsi_pair2
!=======================================================================
subroutine hpsi_new_pair2(zCt_t,zhCt_t)
  use global_variables
  implicit none
  integer :: i
  complex(8),intent(in) :: zCt_t(Lsite)
  ...
  zhCt_t=-gamma*sqrt(2d0*mass*omega0)*0.5d0*(Xp_HO_new + X_HO_new)*zCt_t
