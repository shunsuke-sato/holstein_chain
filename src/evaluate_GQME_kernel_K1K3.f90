...
subroutine evaluate_GQME_kernel_K1K3
  ...
  real(8) :: phi,phi1,phi2,phi3,phi4
  complex(8) :: zfact
...
! off-diagonal (1,j)
  do jsite = 1, Lsite
    ...
    call random_number(phi3); phi3 = 2d0 * pi *phi3
    call random_number(phi4); phi4 = 2d0 * pi *phi4
    do itraj = 1,Ntraj
      if(mod(itraj,Nprocs) /= myrank)cycle

! positive summ
!      call set_initial_conditions_elec
      zC = 0d0; zC(1) = sqrt(0.5d0); zC(jsite) = exp(zI*phi1)*sqrt(0.5d0)
...
! negative summ
!      call set_initial_conditions_elec
!      zC = 0d0; zC(1) = sqrt(0.5d0); zC(jsite) = exp(zI*phi2)*sqrt(0.5d0)
      zC = 0d0; zC(1) = sqrt(0.5d0); zC(jsite) = -exp(zI*phi1)*sqrt(0.5d0)
!      zC(1) = exp(zI*phi3)*zC(1)
!      zC(jsite) = exp(zI*phi4)*zC(jsite)
      call set_initial_conditions_ph(itraj)
      call calc_force_HO
      X_HO_old = X_HO - V_HO*dt +0.5d0*F_HO/mass*dt**2
      zfact = (X_HO(1) - X_HO(jsite))  +zI*0.5d0*beta_KB*(V_HO(1)+V_HO(jsite))
...
! == Kernel calc (1,jsite) ==
      ...
      call MPI_ALLREDUCE(zK3_tmp_l,zK3_tmp,(Nt+1)*Lsite**2 &
        ,MPI_DOUBLE_COMPLEX,MPI_SUM,MPI_COMM_WORLD,ierr)
      zK1_tmp = zK1_tmp/dble(2*Ntraj)*2**2;   zK3_tmp = zK3_tmp/dble(2*Ntraj)*2**2
